...
class _AddTransactionScreenState extends State<AddTransactionScreen> {
  ...
  void _appendDigit(String v) {
    setState(() {
      // guard against silly long input and double dots
  ...
  Future<void> _saveTransaction() async {
    ...
    if (user == null) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('You must be logged in to add transactions')));
      return;
    }
    ...
    if (desc.isEmpty || amountRaw.isEmpty) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Please enter description and amount')));
      return;
    }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: icon == null
            ? const Text(
                '',
                // label text set below for consistency
              )
            : Icon(icon),
      ),
    );
  ...
  // Wrapper to build a keypad key (text or backspace)
